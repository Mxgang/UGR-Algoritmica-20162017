...
//para generar un vector que pueda tener elementos repetidos
  ...
  if (n > TAM_GRANDE){
    ...
    cout << n << "\t" << (tfin-tini)/(double)CLOCKS_PER_SEC << endl; // Mostramos resultados (Tamaño del vector y tiempo de ejecución en seg.)

    cout<< "El tiempo que tarda en ejecutar es: " << " \t  " << ((double) ((t_despues - t_antes) - (t_despues_vacio - t_antes_vacio))) / (CLOCKS_PER_SEC * NUM_VECES) << endl;
    ...
    //cout << "El tamaño del vector es: " << contador << endl;
    int vector[contador];
    for(int i = 0; i<contador; i++){
      vector[i] = U[i];
    }

    cout << "El vector resultante sin repetidos es: ";
